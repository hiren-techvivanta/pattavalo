...
const ProductCom = () => {
  ...
  const [products, setProducts] = useState([]);
  const [subcategories, setSubcategories] = useState([]);
  const [directProducts, setDirectProducts] = useState([]); // New state for direct category products
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [searchResults, setSearchResults] = useState([]);
  const [isSearching, setIsSearching] = useState(false);
  ...
  useEffect(() => {
    if (categories.length > 0 && urlFilters.category) {
      ...
      if (firstCategory?.id) {
        ...
        setParentCategory(firstCategory.category || firstCategory.title);
        setExpandedPanel(`panel1`);
        // Load both subcategories and direct products
  ...
  // Modified function to fetch both subcategories and direct products
  const fetchSubcategoriesAndDirectProducts = async (categoryId) => {
    try {
      ...
      // Fetch direct products under category (not under subcategory)
      ...
      if (
        ...
        // Filter products that don't have subcategory_id (direct under category)
        directProductsData = directProductsResponse.data.data
          ...
          .map((product) => ({
            ...
            subcategoryId: product.subcategory_id,
            apiData: product,
            isDirectProduct: true, // Flag to identify direct products
          }));
      }
      ...
      console.error("Error fetching subcategories and products:", err);
      setSubcategories([]);
      setDirectProducts([]);
      if (err.response) {
        setError(
  ...
  // Function to handle direct product click
    ...
    <div className="min-h-screen bg-white px-4 md:px-12 py-10">
          ...
          <motion.input
            ...
            onChange={(e) => {
              ...
              if (query.trim() === "") {
                setSearchResults([]);
                setViewMode(
                  subcategories.length > 0 || directProducts.length > 0 ? "categories" : "products"
                );
                setShowDetails(false);
              } else {
                setViewMode("search");
      ...
      <div className="mt-2 flex flex-col md:flex-row gap-5">
        ...
        <motion.div
          ...
          className="w-full md:w-1/4"
        >
          {/* Your existing accordion code stays the same */}
          <div className="p-4 rounded-lg">
        ...
        <motion.div
          ...
          className="w-full md:w-3/4"
        >
          {/* Show subcategories and direct products when viewMode is "categories" */}
          {viewMode === "categories" && (
            <div>
