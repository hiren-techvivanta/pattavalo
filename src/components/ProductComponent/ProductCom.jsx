...
//         `${import.meta.env.VITE_BACKEND_URL}/product/subcategory?category_id=${categoryId}`,
//         getAxiosConfig()
//       );

//       let subcategoriesData = [];
//       if (
//         subcategoryResponse.data.message === "Subcategories fetched successfully" &&
//         subcategoryResponse.data.data
//       ) {
//         subcategoriesData = subcategoryResponse.data.data;
//       }

//       const directProductsResponse = await axios.get(
//         `${import.meta.env.VITE_BACKEND_URL}/product/product/search?search=&category_id=${categoryId}`,
//         getAxiosConfig()
//       );

//       let directProductsData = [];
//       if (
//         directProductsResponse.data.message === "Products fetched successfully" &&
...
//       } else if (err.request) {
//         setError(
//           "Network error while loading data. Please try again."
//         );
//       } else {
...
//         await handleProductByUrl(filters.product, category);
...
//           <div className="h-[3px] bg-[#BABEC8]"></div>
//         </div>
...
//                               fontStyle: "normal",
//                               color:
//                                 expandedPanel === `panel${index + 1}` && !selectedSubcategoryId && !selectedProductId
//                                   ? "#2E437C"
//                                   : "#000",
...
//                                           fontStyle: "normal",
//                                           color:
//                                             selectedSubcategoryId === child.subcategoryId && !selectedProductId
//                                               ? "#2E437C"
//                                               : "#666666",
//                                           transition: "color 0.2s ease-in-out",
//                                           whiteSpace: "nowrap",
...
//                                                   fontWeight: 500,
//                                                   fontStyle: "normal",
//                                                   color: selectedProductId === subChild.productId 
//                                                     ? "#2E437C" 
//                                                     : "#6b7280",
//                                                   marginLeft: "2px",
//                                                   transition:
...
//                                         fontWeight: 500,
//                                         fontStyle: "normal",
//                                         color: selectedProductId === child.productId 
//                                           ? "#2E437C" 
//                                           : "#4b5563",
//                                         marginLeft: "2px",
//                                         transition: "color 0.2s ease-in-out",
...
//                         className="flex flex-col items-center text-center cursor-pointer p-4 rounded-lg hover:shadow-md transition-shadow duration-300 border border-gray-100"
//                       >
//                         <motion.img
//                           src={
...
//               {subcategories.length === 0 && directProducts.length === 0 && !subcategoriesLoading && (
//                 <motion.div
...
//                     This category doesn't have any subcategories or products available
//                   </p>
//                 </motion.div>
//               )}
//             </div>
...
const ProductCom = () => {
  ...
  useEffect(() => {
    ...
    const currentUrlParams = `${categoryParam || ""}-${subcategoryParam || ""}-${productParam || ""}`;
    
    // Skip if we've already processed these exact URL parameters
    if (hasProcessedUrlRef.current && lastUrlParamsRef.current === currentUrlParams) {
      return;
    }
  ...
  const updateUrlParams = (params) => {
    ...
    const currentUrlParams = `${searchParams.get("category") || ""}-${searchParams.get("subcategory") || ""}-${searchParams.get("product") || ""}`;
    const newUrlParamsFormatted = `${params.category || ""}-${params.subcategory || ""}-${params.product || ""}`;
  ...
  const applyUrlFilters = async (filters) => {
    try {
      if (filters.category) {
        const category = categories.find(cat => cat.id.toString() === filters.category.toString());
        ...
        if (filters.product) {
          ...
          setSelectedProductId(filters.product);
          setSelectedSubcategoryId(filters.subcategory || null);
          await handleProductByUrl(filters.product, category, filters.subcategory);
        } else if (filters.subcategory) {
  ...
  const handleProductByUrl = async (productId, category, subcategoryId = null) => {
    try {
      setProductDetailsLoading(true);
    ...
    try {
      ...
      if (category.children) {
        category.children.forEach((child, index) => {
          if (child.isSubCategory && child.subcategoryId.toString() === subcategoryId.toString()) {
