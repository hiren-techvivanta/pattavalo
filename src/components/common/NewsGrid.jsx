...
//           className={screen === "home" ? `flex flex-wrap ${gridCols} ${gap}` : `grid ${gridCols} ${gap}`}
...
const NewsGrid = ({
  ...
  const getLayoutClasses = () => {
    if (screen === "home") {
      return {
        container: "md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:gap-6 lg:gap-8",
            ...
            <div className={layoutClasses.container}>
              ...
              {error && !loading && showErrorState && (
                <motion.div
                  ...
                  transition={{ type: "spring", stiffness: 300, damping: 25, duration: 0.6 }}
                >
                  <motion.div className="text-gray-300 mb-6" animate={{ y: [0, -5, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}>
                    <svg className="w-20 h-20 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                  </motion.div>
                  <h3 className="text-xl font-semibold text-gray-500 mb-2">Unable to load posts</h3>
                  <p className="text-gray-400">Please check your connection and try again later.</p>
                </motion.div>
              )}
              ...
              {!loading && !error && filteredData.length === 0 && (
                ...
                >
                  ...
                  <motion.h3 className="text-xl font-semibold text-gray-500 mb-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
                    No posts available
                  </motion.h3>
                  <motion.p className="text-gray-400" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
                    {searchQuery ? `No posts match "${searchQuery}" in ${activeTab}` : `No posts available in ${activeTab} at the moment.`}
                  </motion.p>
                </motion.div>
              )}
            </div>
